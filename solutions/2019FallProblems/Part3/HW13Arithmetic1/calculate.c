...
bool calculate(List * arithlist)
{
  ...
  for(;temp != NULL;)
  {
    ...
    else
    {
      ...
      {
        ListNode *rOp = temp -> prev;
        ListNode *lOp = temp -> prev -> prev;
        int left = atoi(lOp -> word);
        int right = atoi(rOp -> word);

        int ans = 0;
        
        deleteNode(arithlist, rOp);
        deleteNode(arithlist, lOp);
        ...
        memset(temp->word, '\0', sizeof(char)*WORDLENGTH);
        sprintf(temp->word, "%d\n", ans);
        temp = temp->next;      
      }
      else
  ...
  if(arithlist->head->next != NULL) return false;
